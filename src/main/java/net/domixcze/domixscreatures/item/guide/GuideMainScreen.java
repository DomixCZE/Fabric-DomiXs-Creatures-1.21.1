...
public class GuideMainScreen extends Screen {
    ...
    private enum ImageType {
        ITEM,
        TEXTURE,
        CYCLING_ITEM, // New type for cycling items
        CYCLING_TEXTURE // New type for cycling textures
    }

    public record ImageEntry(ImageType type, @Nullable Identifier identifier, @Nullable ItemStack itemStack,
                             @Nullable List<Identifier> cyclingIdentifiers, // New: List of identifiers for cycling textures
                             @Nullable List<ItemStack> cyclingItemStacks, // New: List of ItemStacks for cycling items
                             int frameDurationTicks, // New: Duration for each frame in ticks
                             int x, int y, int width, int height) {

        // Original static constructors, updated to pass nulls/0 for new fields
        ...
        // NEW: Static constructor for cycling textures
        public static ImageEntry cyclingTexture(List<Identifier> identifiers, int frameDurationTicks, int x, int y, int width, int height) {
            // Ensure immutability for the list by copying it
        ...
        // NEW: Static constructor for cycling items
    ...
    @Override
    public void tick() {
        super.tick(); // Call superclass tick method for widgets etc.
        globalImageCycleTick++; // Increment the global timer for image cycling
    }
...
public class GuideMainScreen extends Screen {
    ...
    public record ImageEntry(ImageType type, @Nullable Identifier identifier, @Nullable ItemStack itemStack, int x, int y, int width, int height) {
        public static ImageEntry texture(Identifier identifier, int x, int y, int width, int height) {
            return new ImageEntry(ImageType.TEXTURE, identifier, null, x, y, width, height);
        }

        public static ImageEntry item(ItemStack itemStack, int x, int y, int width, int height) {
            return new ImageEntry(ImageType.ITEM, null, itemStack, x, y, width, height);
        }
    }
