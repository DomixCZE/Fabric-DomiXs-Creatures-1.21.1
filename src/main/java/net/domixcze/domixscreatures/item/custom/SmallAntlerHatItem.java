...
import net.domixcze.domixscreatures.item.client.SmallAntlerHatRenderer;
...
public class SmallAntlerHatItem extends ArmorItem implements GeoItem {
    private final AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    public SmallAntlerHatItem(RegistryEntry<ArmorMaterial> material, Type type, Settings settings) {
    ...
    public void createGeoRenderer(Consumer<GeoRenderProvider> consumer) {
        consumer.accept(new GeoRenderProvider() {
            private SmallAntlerHatRenderer renderer;
            ...
            public <T extends LivingEntity> BipedEntityModel<?> getGeoArmorRenderer(@Nullable T livingEntity, ItemStack itemStack, @Nullable EquipmentSlot equipmentSlot, @Nullable BipedEntityModel<T> original) {
                if (this.renderer == null)
                    this.renderer = new SmallAntlerHatRenderer();
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        ...
        if (color.isPresent()) {
            tooltip.add(Text.translatable("domixs-creatures.tooltip.small_antler")
