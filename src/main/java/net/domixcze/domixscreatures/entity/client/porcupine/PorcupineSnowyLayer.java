...
public class PorcupineSnowyLayer extends GeoRenderLayer<PorcupineEntity> {
    public PorcupineSnowyLayer(GeoRenderer<PorcupineEntity> entityGeoRenderer) {
        super(entityGeoRenderer);
    }

    @Override
    public void render(MatrixStack poseStack, PorcupineEntity animatable, BakedGeoModel bakedModel, RenderLayer renderType, VertexConsumerProvider bufferSource, VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {

        if (animatable.hasSnowLayer()) {
            String texturePath = "textures/entity/";
            String snowTexture = animatable.isBaby() ? "baby_porcupine_snowy.png" : "porcupine_snowy.png";
